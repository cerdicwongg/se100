...
function CurrencyDropdown() {
  ...
  useEffect(() => {
    ...
    setCurrencyList([["USD", "United States Dollar"], ["SGD", "Singapore Dollar"]]);
  }, []);
  ...
  return (
    <div>
      <div className="container">
        ...
        <select name="currency" id="currencySelect" onChange={(e) => {setCurrencyFrom(e.target.value);}} value={currencyFrom}>
          {currencyList.map((currencyItem) => (
            <option value={currencyItem[0]}>{currencyItem[1]}</option>
          ))}
        </select>
        <p>to</p>
        <select name="currency" id="currencySelect" onChange={(e) => {setCurrencyTo(e.target.value);}} value={currencyTo}>
          {currencyList.map((currencyItem) => (
            <option value={currencyItem[0]}>{currencyItem[1]}</option>
          ))}
        </select>
